...
const Info: React.FC<IInfoProps> = ({ followers, following, username, type }) => {
    return (
        <div className="flex justify-start items-center gap-2">
            ...
            <Followers id={username} type={type} initial={followers} />
            {following !== undefined && (
                <Fragment>
                    <span className="w-[4px] h-[4px] rounded-full bg-profile-primary" />
                    <Following id={username} type={type} initial={following} />
                </Fragment>
            )}
        </div>
    );
