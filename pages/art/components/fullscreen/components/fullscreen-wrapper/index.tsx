...
const FullscreenWrapper: React.FC<IFullscreenWrapperProps> = ({ fullscreenArt }) => {
    ...
    return (
        <motion.div
            className="fixed z-[100] top-0 left-0 w-full h-full bg-[rgba(10,10,10,1)] [perspective:1000px] cursor-pointer"
        ...
        >
            <motion.div className="w-full h-full flex justify-center items-center [perspective:1000px]" variants={wrapperVariants} initial="hide" animate="show" exit="exit">
                <motion.div
                ...
                >
                    <Image src={SampleArt} alt={fullscreenArt.title} quality={100} className="rounded-[25px]" style={{ objectFit: "contain" }} priority fill />
                </motion.div>
            </motion.div>
        </motion.div>
    );
